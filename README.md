...
- 非常にシンプルなインターフェース
    - BGMやSEの操作はゲーム内では頻出である為、これを簡潔に扱えるのとそうでないのとではゲーム作成コストに結構な差が出る。なので、後述している複雑な状態をシステム内部で厳密に管理しつつも、実際のBGM/SE再生指示は非常に簡潔に行えるようにした。
...
- https://outcloud.blogspot.jp/2015/11/htmlaudio.html を見て、モバイル対応を補強する
- ロゴ画像をもっと良いものに作り直す
