...
(defdesc :make-play-se-periodically
  (p "エンジン音、プロペラ回転音、機関銃音のような"
     "「動作中は定期的に再生する」タイプのSEの為に、"
     "「動作中は定期的に実行するが、実行が集中しないように、指定したsec間隔"
     "以内での再生を抑制する」関数を生成する高階関数。")
  (p "最初の引数として「抑制する秒数」を渡す。")
  (p "これは「エンジンが加速中の間は毎フレーム鳴らす(再生関数を実行する)が、"
     "(ユーザの入力次第で)加速が停止したら鳴らさない」ような状況で"
     "利用するとよい"
     "(もしそうではなく、終了時に明示的に停止させる方が望ましい場合は、"
     "専用チャンネルでループBGMとして再生させた方がより良い)。")
  (p "再生するSEのパラメータは、"
     "高階関数のオプショナル引数として渡してもよいし、"
     "生成された関数の方の引数として渡してもよい"
     "(生成された方に何も渡されなかった場合は、"
     "高階関数のオプショナル引数が適用される)。")
  (p "再生されたか抑制されたかは、返り値として nil / null が返るか、"
     "SEチャンネルID値が返るかで判定できる。")
  )

(defdesc :make-play-se-personally
  (p "一人のキャラが複数のボイスを再生するような場合は、"
     "同時再生を一つだけに抑制したいケースがほとんど。"
     "これを実現する関数を、再生元(この例ではキャラ)ごとに生成する高階関数。")
  (p "具体的には、あるボイスを再生中に別のボイスの再生を行おうとした"
     "タイミングで、先に再生していたボイスが自動的に停止させられるようになる。")
...
(defdesc :bgm-position
  ...
  (p "再生時に" [:code "pitch"] "を指定していても、"
     "常に pitch=1 での秒数として返される。"
     "また、再生時に" [:code "position"] "を指定していても、"
     "常に音源の先頭からの秒数として返される。"
     )
  (p "※この機能はまだ動作検証が不十分であり、不具合のある可能性がある。")
  )
              ...
              [:div#main-content
               ...
               [:div#main {:style "display: none"}
                ...
                [:div
                 ...
                 (demo-button2 :make-play-se-periodically)
                 (demo-button2 :make-play-se-personally)
                 ]
                ;; prefetch
                [:hr]
                [:div
                 ...
                 [:dl
                  ...
                  [:dd
                   ...
                   "このリストに追加されてしまう為)。"
                   [:br]
                   "この際に、ドットはじまりのファイル名は自動的に除外される"
                   "(" [:code ".htaccess"] "や" [:code ".DS_Store"] "への"
                   "対策としての挙動)。"
                   ]
                  [:dt
                   [:code
                ...
                [:div
                 ...
                 [:ul
                  [:li
                   "ブラウザによってはmp3の特定のエンコードオプション(abr等)を"
                   "解釈できずにエラーになるものがあるようです。"
                   "問題の出るmp3ファイルは、abr等のオプションなしの"
                   "固定ビットレートでエンコードし直すとよいでしょう。"]]
                 [:h4
                  "再生環境によっては、"
                  "BGMのループの際に少しだけ無音の時間がはさまる"]
                 [:ul
                  [:li
                   "HtmlAudioモードで動作する古いモバイル環境で、"
                   "この問題が起こるようです。"
                   [:br]
                   "再生環境側の問題なので、"
                   "ライブラリレベルでの対応は厳しいです。"]]
