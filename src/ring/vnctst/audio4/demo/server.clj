...
(defdesc :stop-bgm-ch-a
  ...
  (p "この例では「" [:code (pr-str "BGS")] "」だけが停止される。")
  )

(defdesc :stop-bgm-ch-b
  (p "第二引数で指定した「BGM再生チャンネルID」で再生中のBGMだけを、"
     "第一引数で指定したフェード秒数かけて終了する。")
  (p "この例では「" [:code "0"] "」、つまりチャンネル無指定でのBGM再生だけが"
     "停止される。")
  )
